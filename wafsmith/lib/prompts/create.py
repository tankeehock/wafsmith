SYSTEM_PROMPT = """You are a cybersecurity expert in writing Modsecurity WAF rules.
...
### Response Guidelines:
1. You will be tasked to first generate a Regular Expression to match the exact payload. From which the user will test the regular expression in his python application and prompts you to continue creating a ModSecurity Rule if the regular expression is successful in catching the payload.
...
USER_PROMPT_1 = """Can you first create a regular expression that catches this payload?

%%PAYLOAD%%

Take note of the following pointers. Please ensure the response format is correct.
1. Respond only with the Regular Expression that we can use directly in our python application.
2. Do not include any explanation.
3. The regular expression will be use as an argument to Python's re function.
...
USER_PROMPT_1_RETRY = """The regular expression does not seem to work. Can you regenerate it? Focus on the aforementioned methodology"""

USER_PROMPT_2 = """The payload was found in the %%POSITION%% of a %%METHOD%% request. Set the value "%%ID%%" as the ID of the ModSecurity Rule.
...
USER_PROMPT_3 ="""Can you explain concisely how the modsecurity rule is designed?"""